...
namespace RectangularCoordinates
{
    ...
    /// ## Further information
    /// 
    /// Here is an example of a tree drawn using rectangular coordinates:
    /// 
    /// <p align="center">
        ...
        public static Dictionary<string, Point> GetCoordinates(TreeNode tree, Dictionary<string, object> parameterValues)
        {
            ...
            if (coordinateShift > 0 && (xShift || yShift))
            {
                foreach (TreeNode node in tree.GetChildrenRecursiveLazy())
                {
                    ...
                    if (xShift)
                    {
                        if (node.Attributes.TryGetValue(xShiftAttribute, out object xAttribute) && xAttribute is double dX)
                        {
                    ...
                    if (yShift)
                    {
                        if (node.Attributes.TryGetValue(yShiftAttribute, out object yAttribute) && yAttribute is double dY)
                        {
                            if (coordinateShift == 1)
                            {
                                y += dY;
            ...
            if (!storedPos.ContainsKey(Modules.RootNodeId))
            {
                double rootX = storedPos[nodes[0].Id].X - Math.Cos(rotation) * rootLength / totalLength * width;
                double rootY = storedPos[nodes[0].Id].Y - Math.Sin(rotation) * rootLength / totalLength * width;

                storedPos[Modules.RootNodeId] = new Point(rootX, rootY);
            }
