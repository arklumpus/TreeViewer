        ...
        public static void Transform(ref TreeNode tree, Dictionary<string, object> parameterValues, Action<double> progressAction)
        {
            ...
            foreach (TreeNode node in nodes)
            {
                ...
                if (matched)
                {
                    ...
                    else if (attrType == "String" && replacementType == "Number" && regex)
                    {
                        string testValue = reg.Replace(((string)attributeValue), replacementValue);
                        if (double.TryParse(testValue, System.Globalization.CultureInfo.InvariantCulture, out _))
                        {
                            currentReplacementValue = testValue;
                        }
                    }
