...
{
    class PNGCommand : Command
    {
        public override ConsoleTextSpan[] HelpText => new ConsoleTextSpan[]
          {
            new ConsoleTextSpan("  png ", 2, ConsoleColor.Green),
            new ConsoleTextSpan("<file name>\n", 2, ConsoleColor.Blue),
            new ConsoleTextSpan("    Saves a plot of the tree in PNG format with the specified ", 4),
            new ConsoleTextSpan("file name", 4, ConsoleColor.Blue),
            new ConsoleTextSpan(".\n\n", 4),

            new ConsoleTextSpan("  png ", 2, ConsoleColor.Green),
            new ConsoleTextSpan("stdout\n", 2, ConsoleColor.Yellow),
            new ConsoleTextSpan("    Writes a plot of the tree in PNG format to the standard output.\n\n", 4),

            new ConsoleTextSpan("  png\n", 2, ConsoleColor.Green),
            new ConsoleTextSpan("    Outputs a plot of the tree in PNG format to the same destination as the previous ", 4),
            new ConsoleTextSpan("png ", 4, ConsoleColor.Green),
            new ConsoleTextSpan("command (default: standard output).", 4),
          };

        public override ConsoleTextSpan[] ShortHelpText => new ConsoleTextSpan[]
        {
            new ConsoleTextSpan("Plots the tree in PNG format.")
        };

        public override string PrimaryCommand => "png";
        ...
        private void OutputPNGToStream(Stream sr)
        {
            Page pag = RenderPlotToPage();

            SixLabors.ImageSharp.Image<SixLabors.ImageSharp.PixelFormats.Rgba32> image = pag.SaveAsImage(Program.Scale);

            image.Metadata.HorizontalResolution = Program.DPI;
            image.Metadata.VerticalResolution = Program.DPI;

            SixLabors.ImageSharp.ImageExtensions.SaveAsPng(image, sr);
        }
        ...
        public override IEnumerable<(ConsoleTextSpan[], string)> GetCompletions(string partialCommand)
        {
            ...
            {
                yield return (new ConsoleTextSpan[] { new ConsoleTextSpan("png", ConsoleColor.Green) }, "png ");
                yield return (new ConsoleTextSpan[] { new ConsoleTextSpan("png ", ConsoleColor.Green), new ConsoleTextSpan("stdout", ConsoleColor.Yellow) }, "png stdout ");
